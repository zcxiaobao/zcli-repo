...
import { ESLINT_FILE_EXT } from "../scanInitDetail.js";
import { getESLintConfigType } from "./getESLintConfigType.js";

/**
 * 获取 ESLint 配置
 */
export function getESLintConfig(opts, pkg, config) {
  ...
  if (config.eslintOptions) {
    ...
    const lintConfigFiles = glob.sync(".eslintrc?(.@(js|yaml|yml|json))", {
      cwd,
    });
    ...
    if (!fs.existsSync(lintIgnoreFile) && !pkg.eslintIgnore) {
      lintConfig.ignorePath = path.resolve(
        __dirname,
        "../config/_eslintignore.ejs"
      );
    }
  }
